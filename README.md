...
将 CHG iTruck Optimizer 文件夹复制到本地。
双击 CHG iTruck Optimizer.exe 启动软件。
确保 image 文件夹与程序在同一目录。


功能概述
CHG iTruck Optimizer 提供以下核心模块：

数据预处理与质量检查：清洗 OTM 原始数据，处理缺失值和异常值。
费用计算：月租车与外叫车成本分析，特殊路线费用计算。
时间利用率分析：计算月租车的时间利用率。
行程拼接：外叫车行程拼接，减少空载。
费用仿真：模拟 LTC 转月租车的费用对比。
预测模块：基于业务需求预测未来运输情况。
...
从 OTM Power BI 导出数据（选择 Data with current layout）。
在软件中：
...
点击 Modify Default Paths 修改默认数据源路径（城市-基地表、部门表、费用表等）。
